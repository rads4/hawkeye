...
function MiniPathPreview({ summary }) {
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap mt-2">
      {hops.map((hop, i) => {
        // Try to guess type from hop name
        const type = hop.toLowerCase().includes('internet') ? 'Internet'
          ...
          : hop.toLowerCase().includes('sg') || hop.toLowerCase().includes('fw') || hop.toLowerCase().includes('vpc') || hop.toLowerCase().includes('subnet') ? 'Network'
          : hop.toLowerCase().includes('s3') || hop.toLowerCase().includes('bucket') || hop.toLowerCase().includes('data') || hop.toLowerCase().includes('pii') ? 'Data'
          : 'Compute';
          ...
          {findings.map((finding, idx) => {
            ...
            return (
              <motion.button
                ...
                transition={{ delay: finding.isNew ? 0 : idx * 0.06, type: 'spring', stiffness: 300, damping: 28 }}
